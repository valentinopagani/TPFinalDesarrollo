...
import { MessageResponse, SafeUser } from '../models/user';
...
export class AuthService {
  ...
  verifyEmail(token: string): Observable<MessageResponse> {
    return this.http.post<MessageResponse>(`${this.api}/verify-email`, { token });
  }

  resendVerification(email?: string | null): Observable<MessageResponse> {
    const body = email ? { email } : {};
    return this.http.post<MessageResponse>(`${this.api}/resend-verification`, body).pipe(
